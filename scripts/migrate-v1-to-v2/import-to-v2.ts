 ...
 *   DEACTIVATE_STALE_ENROLLMENTS  "false" to skip (default: on) — sets other ACTIVE StudentClass
 *                                 rows for the same student+academic year to INACTIVE when V1 assigns a class
 *   IMPORT_PHASE2_GRADES            "true" to import grades.json (large)
 *   IMPORT_PHASE2_ATTENDANCE        "true" to import attendance.json
 *   IMPORT_PHASE2_MONTHLY_SUMMARIES "true" to import student_monthly_summaries.json (monthly scores)
 *   IMPORT_INCLUDE_PARENTS          "true" to import parents + student_parents
 *   IMPORT_INCLUDE_USERS             "true" to import users.json (NOT recommended — Stunity expects new logins)
 *   IMPORT_INCLUDE_GRADE_CONFIRMATIONS "true" to import grade_confirmations (requires User rows for confirmedBy)
 *   IMPORT_INCLUDE_AUDIT_LOGS       "true" to import audit_logs (requires users)
 *   IMPORT_FULL_V1                  "true" preset: parents + all phase-2 bulk (scores, attendance, monthly summaries).
 *                                   Does NOT import login accounts (users) or grade confirmations — students/teachers
 *                                   create or claim accounts in Stunity separately unless IMPORT_INCLUDE_USERS=true.
 */
...
// Preset: school data (rosters, parents, grades, attendance, monthly summaries) — no User/login migration
if (process.env.IMPORT_FULL_V1 === "true") {
  const def = (key: string, val: string) => {
    if (process.env[key] === undefined) process.env[key] = val;
  };
  def("IMPORT_INCLUDE_PARENTS", "true");
  def("IMPORT_PHASE2_GRADES", "true");
  def("IMPORT_PHASE2_ATTENDANCE", "true");
  def("IMPORT_PHASE2_MONTHLY_SUMMARIES", "true");
}

// ─── Config ──────────────────────────────────────────────────────────────────

const IMPORT_DIR =
  process.env.IMPORT_DIR || path.join(__dirname, "data", "export-latest");
...
const DEACTIVATE_STALE_ENROLLMENTS =
  process.env.DEACTIVATE_STALE_ENROLLMENTS !== "false";
const IMPORT_PHASE2_GRADES = process.env.IMPORT_PHASE2_GRADES === "true";
const IMPORT_PHASE2_ATTENDANCE =
  process.env.IMPORT_PHASE2_ATTENDANCE === "true";
const IMPORT_PHASE2_MONTHLY_SUMMARIES =
  process.env.IMPORT_PHASE2_MONTHLY_SUMMARIES === "true";
const IMPORT_INCLUDE_PARENTS = process.env.IMPORT_INCLUDE_PARENTS === "true";
const IMPORT_INCLUDE_USERS = process.env.IMPORT_INCLUDE_USERS === "true";
const IMPORT_INCLUDE_GRADE_CONFIRMATIONS =
  process.env.IMPORT_INCLUDE_GRADE_CONFIRMATIONS === "true";
const IMPORT_INCLUDE_AUDIT_LOGS =
  process.env.IMPORT_INCLUDE_AUDIT_LOGS === "true";
const IMPORT_SCORE_START_MONTH = process.env.IMPORT_SCORE_START_MONTH || null; // YYYY-MM
const IMPORT_SCORE_END_MONTH = process.env.IMPORT_SCORE_END_MONTH || null; // YYYY-MM
const IMPORT_SCORE_SOURCE_YEAR_MODE =
  process.env.IMPORT_SCORE_SOURCE_YEAR_MODE || "as-is";
...
interface MigrationReport {
  ...
  completedAt?: string;
  durationMs?: number;
  /** So validate-migration can skip row-count checks for tables not migrated */
  importOptions?: {
    importedParents: boolean;
    importedGrades: boolean;
    importedAttendance: boolean;
    importedMonthlySummaries: boolean;
    importedUserAccounts: boolean;
    importedGradeConfirmations: boolean;
  };
}
...
const VALID_WORKING_LEVELS = new Set([
  "FRAMEWORK_A",
  "FRAMEWORK_B",
  "FRAMEWORK_C",
  "CONTRACT",
...
const VALID_ACCOUNT_TYPES = new Set<AccountTypeValue>([
  "SOCIAL_ONLY",
  "SCHOOL_ONLY",
  "HYBRID",
]);
...
function inferAccountType(user: any): AccountTypeValue {
  const explicit =
    typeof user.accountType === "string"
      ? user.accountType.toUpperCase().trim()
      : "";
...
function pad(label: string, width = 32) {
...
function parseYearMonth(value: string | null): number | null {
  ...
  if (!m) throw new Error(`Invalid YYYY-MM value: ${value}`);
...
const SCORE_START_VALUE = parseYearMonth(IMPORT_SCORE_START_MONTH);
const SCORE_END_VALUE = parseYearMonth(IMPORT_SCORE_END_MONTH);

function scoreMonthNumber(row: any): number | null {
  const numeric = Number(row.monthNumber);
  if (Number.isInteger(numeric) && numeric >= 1 && numeric <= 12)
    return numeric;
  const raw = String(row.month ?? "").trim();
  if (!raw) return null;
  return MONTH_NUMBERS[raw] ?? MONTH_NUMBERS[raw.toLowerCase()] ?? null;
}

function scorePeriod(row: any): { year: number; monthNumber: number } | null {
  const monthNumber = scoreMonthNumber(row);
  const rawYear = Number(row.year);
  if (!monthNumber || !Number.isInteger(rawYear)) return null;

  let year = rawYear;
  if (
    IMPORT_SCORE_SOURCE_YEAR_MODE === "academic-start" &&
    monthNumber >= 1 &&
    monthNumber <= 8
  ) {
    year = rawYear + 1;
...
function scorePeriodInRange(period: { year: number; monthNumber: number }) {
  const value = period.year * 100 + period.monthNumber;
  if (SCORE_START_VALUE !== null && value < SCORE_START_VALUE) return false;
  if (SCORE_END_VALUE !== null && value > SCORE_END_VALUE) return false;
  return true;
}

function scoreRowIncluded(row: any) {
  const period = scorePeriod(row);
  return period ? scorePeriodInRange(period) : false;
}
...
async function main() {
  ...
  const monthlySummaries =
    loadJson<any[]>("student_monthly_summaries.json") || [];
  ...
  if (DRY_RUN) {
    const yearStrings = [
      ...new Set(classes.map((c) => c.academicYear || "2024-2025")),
    ];
    console.log("  🔍 DRY RUN Preview:");
    console.log(`    Would create school       : "${SCHOOL_NAME}"`);
    console.log(`    AcademicYears to create   : ${yearStrings.join(", ")}`);
    console.log(
      `    GradingScales (per year)  : ${yearStrings.length} (Cambodian A–F scale)`,
    );
    ...
    console.log(
      `    StudentClass enrollments  : ${students.filter((s) => s.classId).length}`,
    );
    ...
    console.log(
      `    DEACTIVATE_STALE_ENROLLMENTS: ${DEACTIVATE_STALE_ENROLLMENTS}`,
    );
    console.log(
      `    Phase 2 bulk              : grades=${IMPORT_PHASE2_GRADES} attendance=${IMPORT_PHASE2_ATTENDANCE} monthlySummaries=${IMPORT_PHASE2_MONTHLY_SUMMARIES}`,
    );
    if (IMPORT_SCORE_START_MONTH || IMPORT_SCORE_END_MONTH) {
      console.log(
        `    Score window              : ${IMPORT_SCORE_START_MONTH || "(start)"} → ${IMPORT_SCORE_END_MONTH || "(end)"}`,
      );
      console.log(
        `    Score year mode           : ${IMPORT_SCORE_SOURCE_YEAR_MODE}`,
      );
      console.log(
        `    Grades in score window    : ${grades.filter(scoreRowIncluded).length}`,
      );
      console.log(
        `    Monthly summaries in window: ${monthlySummaries.filter(scoreRowIncluded).length}`,
      );
    }
    console.log(
      `    IMPORT_FULL_V1 preset     : ${process.env.IMPORT_FULL_V1 === "true"}`,
    );
    console.log(`    Include parents           : ${IMPORT_INCLUDE_PARENTS}`);
    console.log(`    Include users             : ${IMPORT_INCLUDE_USERS}`);
    console.log(
      `    Include grade confirmations: ${IMPORT_INCLUDE_GRADE_CONFIRMATIONS}`,
    );
    console.log(`    Include audit logs        : ${IMPORT_INCLUDE_AUDIT_LOGS}`);
    console.log("");
    ...
    console.log(
      `    IMPORT_DIR="${IMPORT_DIR}" CREATE_SCHOOL=true SCHOOL_NAME="${SCHOOL_NAME}" npx tsx scripts/migrate-v1-to-v2/import-to-v2.ts`,
    );
    console.log(
      "  Or align an existing V2 school (recommended for production):",
    );
    console.log(
      `    IMPORT_DIR="${IMPORT_DIR}" SCHOOL_ID="<v2-school-cuid>" npx tsx scripts/migrate-v1-to-v2/import-to-v2.ts`,
    );
    console.log("");
    return;
  }
  ...
  const v2ClassAcademicYearByV1ClassId: Record<string, string> = {};
  ...
  function inc(key: string, by = 1) {
    report.created[key] = (report.created[key] || 0) + by;
  }
  function skip(key: string, by = 1) {
  ...
  function recordError(entity: string, id: string, err: unknown) {
    ...
    if (!SKIP_ERRORS)
      throw new Error(
        `Aborting due to error in ${entity} ${id}. Set SKIP_ERRORS=true to continue.`,
      );
  }
  ...
  try {
    ...
    if (SCHOOL_ID_ENV) {
      const school = await prisma.school.findUnique({
        where: { id: SCHOOL_ID_ENV },
      });
      if (!school) {
        console.error(`❌  School "${SCHOOL_ID_ENV}" not found.`);
        process.exit(1);
      }
      schoolId = school.id;
      console.log(
        `  ✅  ${pad("School (existing)")} ${school.name} [${school.id}]`,
      );
    } else if (CREATE_SCHOOL) {
      const slug =
        SCHOOL_NAME.toLowerCase()
          .replace(/\s+/g, "-")
          .replace(/[^a-z0-9-]/g, "") +
        "-" +
        Date.now();
      const school = await prisma.school.create({
        data: {
          name: SCHOOL_NAME,
          slug,
          email: `migrated-${Date.now()}@school.local`,
      ...
      schoolId = school.id;
      console.log(
        `  ✅  ${pad("School (created)")} ${school.name} [${school.id}]`,
      );
      inc("school");
    } else {
      ...
      if (!school) {
        console.error(
          "❌  No active school found. Use CREATE_SCHOOL=true or SCHOOL_ID=<id>.",
        );
        process.exit(1);
      }
      schoolId = school.id;
      console.log(
        `  ✅  ${pad("School (first active)")} ${school.name} [${school.id}]`,
      );
    }
    ...
    await prisma.$transaction(
      async (tx) => {
        ...
        console.log(
          `  ✅  ${pad("AcademicYears")}${counter(yearStrings.length)}`,
        );
        ...
        for (const ys of yearStrings) {
          ...
          const existing = await (tx as any).gradingScale.findFirst({
        ...
        console.log(
          `  ✅  ${pad("GradingScales (created)")}${counter(gsCreated)}`,
        );
    ...
    for (const s of subjects) {
      if (!s.code) {
        recordError("subject", s.id, "Missing code field");
        continue;
      }
      try {
        ...
        recordError("subject", s.id || s.code, err);
      }
    }
    console.log(
      `  ✅  ${pad("Subjects created/updated")}${counter(subjCreated)} / ${subjUpdated}`,
    );
    inc("subjects", subjCreated);
    skip("subjects", subjUpdated);
    ...
    console.log(
      `  ✅  ${pad("Teachers")}${counter(tchCreated)} created / ${tchSkipped} updated`,
    );
    ...
    for (const c of classes) {
      ...
      if (!ayId) {
        recordError("class", c.id, "AcademicYear not found: " + c.academicYear);
        continue;
      }
      try {
        const homeroomTeacherId = c.homeroomTeacherId
          ? (idMap.teacher[c.homeroomTeacherId] ?? null)
          : null;
        const classData = {
          id: c.id,
          schoolId,
        ...
        v2ClassAcademicYearByV1ClassId[c.id] = result.academicYearId;
        clsCreated++;
      } catch (err) {
    ...
    await upsertInBatches(students, 50, async (s) => {
      ...
      const studentData: any = {
        ...
        accountDeactivatedAt: s.accountDeactivatedAt
          ? new Date(s.accountDeactivatedAt)
          : null,
      ...
      await upsertInBatches(parents, 50, async (p) => {
        ...
        try {
          ...
          const result = await prisma.parent.upsert({
            where: { id: existingParent?.id ?? p.id },
            create: {
              id: p.id,
    ...
    if (IMPORT_INCLUDE_PARENTS) {
      await upsertInBatches(studentParents, 100, async (sp) => {
        ...
        try {
          ...
          recordError(
            "studentParent",
            sp.id || `${sp.studentId}+${sp.parentId}`,
            err,
          );
        }
      });
    ...
    type DesiredEnrollment = {
      studentId: string;
      classId: string;
      academicYearId: string;
    };
    const desiredEnrollments: DesiredEnrollment[] = [];
    const classlessStudentIds: string[] = [];
    ...
    for (const s of students) {
      ...
      if (!s.classId) {
        classlessStudentIds.push(studentId);
        continue;
      }
      ...
      const academicYearId =
        v2ClassAcademicYearByV1ClassId[s.classId] ||
        (cls ? idMap.academicYear[cls.academicYear || "2024-2025"] : null);
      if (!classId || !academicYearId) {
        skip("studentClasses");
        continue;
      }
      desiredEnrollments.push({ studentId, classId, academicYearId });
    }
    ...
    for (const batch of chunks(desiredEnrollments, 500)) {
      const values = batch
        .map(
          (r) =>
            `(${sqlQuote(r.studentId)}, ${sqlQuote(r.classId)}, ${sqlQuote(r.academicYearId)})`,
        )
        .join(", ");

      // Upgrade one legacy null-year row per student/class instead of creating
      // a second ACTIVE row. This keeps current class counts exact.
      scReused += await prisma.$executeRawUnsafe(`
        WITH desired("studentId", "classId", "academicYearId") AS (VALUES ${values}),
        legacy AS (
          SELECT DISTINCT ON (sc."studentId", sc."classId")
            sc.id,
            desired."academicYearId"
          FROM student_classes sc
          JOIN desired
            ON desired."studentId" = sc."studentId"
           AND desired."classId" = sc."classId"
          WHERE sc."academicYearId" IS NULL
            AND NOT EXISTS (
              SELECT 1
              FROM student_classes exact
              WHERE exact."studentId" = desired."studentId"
                AND exact."classId" = desired."classId"
                AND exact."academicYearId" = desired."academicYearId"
            )
          ORDER BY sc."studentId", sc."classId", sc."updatedAt" DESC
        )
        UPDATE student_classes sc
        SET "academicYearId" = legacy."academicYearId",
            status = 'ACTIVE',
            "updatedAt" = NOW()
        FROM legacy
        WHERE sc.id = legacy.id
      `);

      const createResult = await prisma.studentClass.createMany({
        data: batch.map((r) => ({
      ...
      await prisma.$executeRawUnsafe(`
        ...
        SET status = 'ACTIVE',
            "updatedAt" = NOW()
        FROM desired
        WHERE sc."studentId" = desired."studentId"
          AND sc."classId" = desired."classId"
          AND sc."academicYearId" = desired."academicYearId"
      `);
    }

    if (DEACTIVATE_STALE_ENROLLMENTS) {
      if (desiredEnrollments.length > 0) {
        const values = desiredEnrollments
          .map(
            (r) =>
              `(${sqlQuote(r.studentId)}, ${sqlQuote(r.classId)}, ${sqlQuote(r.academicYearId)})`,
          )
        ...
        scDeactivated += await prisma.$executeRawUnsafe(`
          WITH desired("studentId", "classId", "academicYearId") AS (VALUES ${values})
          UPDATE student_classes sc
          SET status = 'INACTIVE',
              "updatedAt" = NOW()
          WHERE sc.status = 'ACTIVE'
            AND sc."studentId" IN (SELECT "studentId" FROM desired)
            AND NOT EXISTS (
              SELECT 1
              FROM desired
              WHERE desired."studentId" = sc."studentId"
                AND desired."classId" = sc."classId"
                AND desired."academicYearId" = sc."academicYearId"
            )
        `);
      }

      for (const ids of chunks(classlessStudentIds, 500)) {
        scDeactivated += (
      ...
      console.log(
        `  ✅  ${pad("Stale enrollments deactivated")}${counter(scDeactivated)}`,
      );
    }
    console.log(
      `  ✅  ${pad("StudentClass aligned")}${counter(desiredEnrollments.length)} (${scCreated} created / ${scReused} reused)`,
    );
    inc("studentClasses", desiredEnrollments.length);
    ...
    await upsertInBatches(subjectTeachers, 10, async (st) => {
      ...
      try {
        ...
        recordError("subjectTeacher", st.id, err);
      }
    });
    ...
    await upsertInBatches(teacherClasses, 50, async (tc) => {
      ...
      try {
        ...
        recordError("teacherClass", tc.id, err);
      }
    });
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // PHASE 2 — Bulk data (opt-in): grades, attendance, monthly summaries
    //   createMany(skipDuplicates); outside transaction.
    // ═══════════════════════════════════════════════════════════════════════
    ...
    async function insertBatch<T>(
      ...
      for (let i = 0; i < mapped.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          console.error(
            `  ⚠️  Batch error in ${label} at offset ${i}: ${err.message?.slice(0, 100)}`,
          );
        }
        if (mapped.length > 5000 && i > 0 && i % 10000 === 0) {
          console.log(
            `    … ${label} ${i.toLocaleString()} / ${mapped.length.toLocaleString()}`,
          );
        }
      }
    ...
    if (
      IMPORT_PHASE2_GRADES ||
      IMPORT_PHASE2_ATTENDANCE ||
      IMPORT_PHASE2_MONTHLY_SUMMARIES
    ) {
      console.log("");
      const flags = [
        IMPORT_PHASE2_GRADES && "grades",
        IMPORT_PHASE2_ATTENDANCE && "attendance",
        IMPORT_PHASE2_MONTHLY_SUMMARIES && "monthlySummaries",
      ].filter(Boolean);
      console.log(`  Phase 2: Bulk data (${flags.join(", ")})…`);
      if (IMPORT_SCORE_START_MONTH || IMPORT_SCORE_END_MONTH) {
        console.log(
          `  Score window: ${IMPORT_SCORE_START_MONTH || "(start)"} → ${IMPORT_SCORE_END_MONTH || "(end)"}`,
        );
        console.log(`  Score year mode: ${IMPORT_SCORE_SOURCE_YEAR_MODE}`);
      }

      if (IMPORT_PHASE2_GRADES) {
        await insertBatch(
          "Grades",
          grades,
          (g) => {
            const period = scorePeriod(g);
            if (!period || !scorePeriodInRange(period)) return null;
            ...
            return {
              ...
              weightedScore: g.weightedScore ?? null,
      ...
      if (IMPORT_PHASE2_ATTENDANCE) {
        await insertBatch(
          ...
          (a) => {
            ...
            if (IMPORT_SCORE_SOURCE_YEAR_MODE === "academic-start") {
      ...
      if (IMPORT_PHASE2_MONTHLY_SUMMARIES) {
        await insertBatch(
          "Monthly Summaries",
          monthlySummaries,
          (m) => {
            const period = scorePeriod(m);
            if (!period || !scorePeriodInRange(period)) return null;
            const studentId = idMap.student[m.studentId];
            const classId = idMap.class[m.classId];
            if (!studentId || !classId) {
              skip("monthlySummaries");
              return null;
            }
            return {
              studentId,
              ...
              totalWeightedScore: m.totalWeightedScore,
              totalCoefficient: m.totalCoefficient,
              average: m.average,
              classRank: m.classRank ?? null,
              gradeLevel: m.gradeLevel ?? null,
            };
          },
          (data) =>
            prisma.studentMonthlySummary.createMany({
              data: data as any,
              skipDuplicates: true,
            }),
          "monthlySummaries",
    ...
    // ── 16. Users (login accounts; run after parents when IMPORT_INCLUDE_PARENTS)
    if (IMPORT_INCLUDE_USERS) {
      let usrCreated = 0;
      for (const u of users) {
        const studentId = u.studentId
          ? (idMap.student[u.studentId] ?? null)
          : null;
        const teacherId = u.teacherId
          ? (idMap.teacher[u.teacherId] ?? null)
          : null;
        const parentId = u.parentId ? (idMap.parent[u.parentId] ?? null) : null;
        const accountType = inferAccountType(u);
        const hashArr = Array.isArray(u.lastPasswordHashes)
          ? u.lastPasswordHashes
          : [];
        try {
          ...
          idMap.user[u.id] = u.id;
          usrCreated++;
        } catch (err) {
          recordError("user", u.id, err);
        }
      }
    ...
    // ── 17. Grade confirmations (after users for confirmedBy FK)
    if (IMPORT_INCLUDE_GRADE_CONFIRMATIONS) {
      let gcCreated = 0;
      for (const gc of gradeConfirmations) {
        const classId = idMap.class[gc.classId];
        const subjectId = idMap.subject[gc.subjectId];
        const confirmedBy = (idMap.user[gc.confirmedBy] ??
          gc.confirmedBy) as string;
        if (!classId || !subjectId || !confirmedBy) {
          skip("gradeConfirmations");
          continue;
        }
        try {
          await prisma.gradeConfirmation.upsert({
            ...
            create: {
              ...
              month: gc.month,
              year: gc.year,
              isConfirmed: gc.isConfirmed ?? false,
              confirmedBy,
            },
            update: { isConfirmed: gc.isConfirmed ?? false, confirmedBy },
          });
          gcCreated++;
        } catch (err) {
          recordError(
            "gradeConfirmation",
            `${gc.classId}-${gc.subjectId}-${gc.month}`,
            err,
          );
        }
      }
      console.log(`  ✅  ${pad("GradeConfirmations")}${counter(gcCreated)}`);
      inc("grade_confirmations", gcCreated);
    }

    // ── 18. Audit logs (optional)
    if (IMPORT_INCLUDE_AUDIT_LOGS) {
      let alCreated = 0;
      for (const al of auditLogs) {
        const adminId = (idMap.user[al.adminId] ?? al.adminId) as string;
        const teacherUserId = (idMap.user[al.teacherId] ??
          al.teacherId) as string;
        if (!adminId || !teacherUserId) {
          skip("auditLogs");
          continue;
        }
        try {
    ...
    report.importOptions = {
      importedParents: IMPORT_INCLUDE_PARENTS,
      importedGrades: IMPORT_PHASE2_GRADES,
      importedAttendance: IMPORT_PHASE2_ATTENDANCE,
      importedMonthlySummaries: IMPORT_PHASE2_MONTHLY_SUMMARIES,
      importedUserAccounts: IMPORT_INCLUDE_USERS,
      importedGradeConfirmations: IMPORT_INCLUDE_GRADE_CONFIRMATIONS,
    };
    ...
    console.log("");
    console.log("  Created:");
    for (const [k, v] of Object.entries(report.created))
      console.log(`    ${pad(k)}${counter(v)}`);
    ...
    console.log(
      `    IMPORT_DIR="${IMPORT_DIR}" npx tsx scripts/migrate-v1-to-v2/validate-migration.ts`,
    );
    console.log("");
  } catch (err) {
    ...
    console.error(
      "    If this happened during Phase 2, structural data was committed — re-run with SKIP_ERRORS=true to resume.\n",
    );
    process.exit(1);
  } finally {
