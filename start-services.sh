...
cd "$SCRIPT_DIR/services/auth-service" && PORT=3001 npm run dev > /tmp/stunity-auth.log 2>&1 &

echo "🏫 Starting School Service (port 3002)..."
cd "$SCRIPT_DIR/services/school-service" && PORT=3002 npm run dev > /tmp/stunity-school.log 2>&1 &

echo "👨‍🎓 Starting Student Service (port 3003)..."
cd "$SCRIPT_DIR/services/student-service" && PORT=3003 npm run dev > /tmp/stunity-student.log 2>&1 &

echo "👨‍🏫 Starting Teacher Service (port 3004)..."
cd "$SCRIPT_DIR/services/teacher-service" && PORT=3004 npm run dev > /tmp/stunity-teacher.log 2>&1 &

echo "📚 Starting Class Service (port 3005)..."
cd "$SCRIPT_DIR/services/class-service" && PORT=3005 npm run dev > /tmp/stunity-class.log 2>&1 &

echo "🌐 Starting Web App (port 3000)..."
cd "$SCRIPT_DIR/apps/web" && PORT=3000 npm run dev > /tmp/stunity-web.log 2>&1 &
