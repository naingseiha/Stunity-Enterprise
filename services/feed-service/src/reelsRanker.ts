...
/**
 * Deterministic shuffle shared with the Learn practice endpoint (see
 * utils/seededShuffle.ts) — same question id ⇒ same option order everywhere,
 * so ReelResponse.chosenIndex stays consistent across surfaces. This wrapper
 * just applies the reels env gate.
 */
function shuffleOptions(
  ...
  if (!SHUFFLE_OPTIONS_ENABLED) return { options, correctAnswer };
  return seededShuffleOptions(options, correctAnswer, seed);
}
