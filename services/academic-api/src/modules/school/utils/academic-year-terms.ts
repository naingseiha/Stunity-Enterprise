...
const normalizedIntegers = (values: unknown, minimum: number, maximum: number) =>
...
const scopesOverlap = (left: number[], right: number[]) =>
  left.length === 0 || right.length === 0 || left.some((grade) => right.includes(grade));
...
export function normalizeAndValidateAcademicTerms(
  ...
  const terms = rawTerms.map((raw: any, index) => {
    ...
    const gradeLevels = normalizedIntegers(raw?.gradeLevels, 1, 12);
    const excludedMonths = normalizedIntegers(raw?.excludedMonths ?? raw?.skipMonths, 1, 12);
...
export const academicTermData = (term: AcademicTermInput) => ({
  name: term.name,
