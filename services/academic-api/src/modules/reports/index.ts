...
const getSchoolId = (req: AuthRequest): string | null =>
  req.user?.schoolId || req.user?.school?.id || null;
const getAuthUserId = (req: AuthRequest): string | null =>
  req.user?.userId || req.user?.id || null;

const SCHOOL_WIDE_REPORT_ROLES = new Set([
  "ADMIN",
  "STAFF",
  "SUPER_ADMIN",
  "SCHOOL_ADMIN",
...
let subjectMetaCache: {
  map: Map<string, SubjectMeta>;
  timestamp: number;
} | null = null;
...
async function getSubjectMetaMap(): Promise<Map<string, SubjectMeta>> {
  if (
    subjectMetaCache &&
    Date.now() - subjectMetaCache.timestamp <= SUBJECT_META_CACHE_TTL_MS
  ) {
...
async function resolveAllowedClassIds(
    ...
    if (requestedClassId) {
      ...
      if (!classData)
        return {
          allowed: false,
          status: 404,
          message: "Class not found in this academic year",
        };
      return {
        allowed: true,
  ...
  if (role !== "TEACHER") {
    return {
      allowed: false,
      status: 403,
      message: "You do not have access to school reports",
    };
  }
  ...
  return {
    allowed: true,
    classIds: ownClassIds,
    scopeClassId: ownClassIds.length === 1 ? ownClassIds[0] : null,
  };
}
...
async function resolvePeriod(
  ...
  if (period === "month") {
    ...
    return {
      ...
      endDate,
      periods,
      label: `${year}-${String(mn).padStart(2, "0")}`,
      khmerLabel: periods[0]?.label || "",
    };
  }
  ...
  if (period === "semester") {
    ...
    if (matchingTerms.length) {
      ...
      for (const term of matchingTerms) {
        for (const reportPeriod of enumerateReportPeriods(term.startDate, term.endDate)) {
          if (!term.excludedMonths.includes(reportPeriod.monthNumber)) {
            periodMap.set(`${reportPeriod.year}-${reportPeriod.monthNumber}`, reportPeriod);
          }
        }
      }
      const periods = Array.from(periodMap.values()).sort(
        (left, right) => left.year * 100 + left.monthNumber - (right.year * 100 + right.monthNumber),
      );
      return {
        startDate,
        endDate,
        periods,
        label: matchingTerms.length === 1 ? matchingTerms[0].name : `Semester ${semesterNumber}`,
        khmerLabel: matchingTerms.length === 1 ? matchingTerms[0].name : `ឆមាសទី${semesterNumber}`,
      };
    }
    const startYear =
      parseAcademicStartYearName(academicYear?.name) ||
  ...
  // period === 'year'
  const startDate =
    academicYear?.startDate || monthStart(new Date().getFullYear(), 1);
  const endDate =
    academicYear?.endDate || monthEnd(new Date().getFullYear(), 12);
  const periods = enumerateReportPeriods(startDate, endDate);
  return {
    startDate,
    endDate,
...
type PosterScope = "class" | "multiClass" | "grade" | "school";
type PosterGroupBy = "class" | "grade" | "none";

type PosterRecipientCandidate = {
  studentId: string;
  name: string;
...
function rankPosterRecipients(
  candidates: PosterRecipientCandidate[],
  limit: number,
  includeTies: boolean,
) {
  const sorted = [...candidates].sort((a, b) => {
    if (b.average !== a.average) return b.average - a.average;
    const aName = a.khmerName || a.name;
    const bName = b.khmerName || b.name;
    return aName.localeCompare(bName, "km");
  });

  let previousAverage: number | null = null;
  let previousRank = 0;
  const ranked = sorted.map((recipient, index) => {
    const rank =
      previousAverage !== null && recipient.average === previousAverage
        ? previousRank
        : index + 1;
    previousAverage = recipient.average;
    previousRank = rank;
    return { ...recipient, rank };
  });

  if (!includeTies || ranked.length <= limit) return ranked.slice(0, limit);
  const threshold = ranked[limit - 1]?.average;
  return threshold === undefined
    ? []
    : ranked.filter((recipient) => recipient.average >= threshold);
}

/**
 * Poster Studio data source. This endpoint is intentionally separate from the
 * analytics dashboard so poster layout/count choices never affect dashboard
 * aggregation or export behavior.
 */
app.get(
  "/reports/poster-recipients",
  authenticateToken,
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!schoolId)
        return res
          .status(400)
          .json({ success: false, message: "School context is required" });
      ...
      const {
        ...
        groupBy: rawGroupBy,
        limit: rawLimit,
        includeTies: rawIncludeTies,
      } = req.query as Record<string, string | undefined>;

      if (!yearId)
        return res
          .status(400)
          .json({ success: false, message: "yearId is required" });

      const scope: PosterScope = [
        "class",
        "multiClass",
        "grade",
        "school",
      ].includes(rawScope || "")
        ? (rawScope as PosterScope)
        : "school";
      const groupBy: PosterGroupBy = ["class", "grade", "none"].includes(
        rawGroupBy || "",
      )
        ? (rawGroupBy as PosterGroupBy)
        : scope === "grade"
          ? "grade"
      ...
      const includeTies = rawIncludeTies !== "false";
      const periodType: PeriodType =
        period === "semester" || period === "year" ? period : "month";
      const requestedClassIds = parseCsvIds(rawClassIds);

      const [access, academicYear, school] = await Promise.all([
        resolveAllowedClassIds(req, schoolId, yearId),
      ...
      const allowedClassIdSet = new Set(access.classIds);
      if (requestedClassIds.some((id) => !allowedClassIdSet.has(id))) {
        return res.status(403).json({
          success: false,
          message:
            "One or more selected classes are not available to this account",
        });
      }
      if (
        (scope === "class" || scope === "multiClass") &&
        ...
        return res.status(400).json({
          success: false,
          message: "At least one class is required for this poster scope",
        });
      }
      if (scope === "class" && requestedClassIds.length > 1) {
        return res.status(400).json({
          success: false,
          message: "Class scope accepts exactly one class",
        });
      }
      ...
      const selectedClasses = allowedClasses.filter((classItem) => {
        if (scope === "class" || scope === "multiClass")
          return requestedClassIds.includes(classItem.id);
        if (scope === "grade") return String(classItem.grade) === String(grade);
        return true;
      });
      const selectedClassIds = selectedClasses.map((classItem) => classItem.id);
      const homeroomTeachers = Array.from(
        new Map(
          selectedClasses
            .filter((classItem) => classItem.homeroomTeacher)
            .map((classItem) => {
              const teacher = classItem.homeroomTeacher!;
              return [
                teacher.id,
                {
                  id: teacher.id,
                  name:
                    (teacher.customFields as any)?.regional?.khmerName ||
      ...
      const homeroomTeacher =
        homeroomTeachers.length === 1 ? homeroomTeachers[0] : null;
      ...
      const term = await resolvePeriod(
        ...
        new Set(selectedClasses.map((item) => Number(item.grade))).size === 1
          ? Number(selectedClasses[0]?.grade)
          : undefined,
      );
      ...
      if (selectedClassIds.length === 0) {
        return res.json({
          ...
          homeroomTeacher,
          scale: resolveGradeScale(school?.educationModel),
          generatedAt: new Date().toISOString(),
        });
      }

      const gradePeriodWhere = buildGradePeriodWhere(term.periods);
      const [students, subjectMeta, studentSubjectGroups] = await Promise.all([
        prisma.student.findMany({
          where: { schoolId, classId: { in: selectedClassIds } },
          select: {
            id: true,
            classId: true,
            firstName: true,
        ...
        prisma.grade.groupBy({
          ...
          where: { classId: { in: selectedClassIds }, ...gradePeriodWhere },
          _sum: { score: true },
          _count: { _all: true },
        }),
      ]);
      ...
      const classMap = new Map(
        selectedClasses.map((classItem) => [classItem.id, classItem]),
      );
      const studentMap = new Map(
        students.map((student) => [student.id, student]),
      );
      const subjectMeansByStudent = new Map<
        string,
        Array<{ mean: number; coefficient: number }>
      >();

      studentSubjectGroups.forEach((group) => {
        const count = group._count._all;
        if (count === 0) return;
        const entries = subjectMeansByStudent.get(group.studentId) || [];
        entries.push({
          mean: (group._sum.score || 0) / count,
          coefficient: subjectMeta.get(group.subjectId)?.coefficient ?? 0,
        });
        subjectMeansByStudent.set(group.studentId, entries);
      });

      const candidates: PosterRecipientCandidate[] = [];
      subjectMeansByStudent.forEach((subjectMeans, studentId) => {
        ...
        candidates.push({
          ...
          khmerName: studentKhmerName(student),
          photoUrl: student.photoUrl,
          classId: classItem.id,
          className: classItem.name,
          grade: String(classItem.grade),
          average:
            Math.round(combineSubjectAverages(subjectMeans, gradeScale) * 100) /
            100,
        });
      });
      ...
      const groupedCandidates = new Map<
        string,
        {
          ...
          label: string;
          type: PosterGroupBy;
          recipients: PosterRecipientCandidate[];
        }
      >();
        ...
        const label =
          groupBy === "class"
            ...
            : groupBy === "grade"
              ? candidate.grade
              : school?.name || "School";
        const bucket = groupedCandidates.get(id) || {
          id,
          label,
          type: groupBy,
          recipients: [],
        };
        bucket.recipients.push(candidate);
        groupedCandidates.set(id, bucket);
      });
      ...
      const groups = Array.from(groupedCandidates.values())
        .map((group) => ({
          ...
          label: group.label,
          type: group.type,
          recipients: rankPosterRecipients(
            group.recipients,
            limit,
            includeTies,
          ),
      ...
      console.error("❌ Error building poster recipients:", error);
      return res.status(500).json({
        success: false,
        message: "Error building poster recipients",
        error: error.message,
      });
    }
  },
...
app.get(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const scopedGrades = await prisma.class.findMany({
        where: { id: { in: classIds }, schoolId, academicYearId: yearId },
        select: { grade: true },
        distinct: ['grade'],
      });
      const term = await resolvePeriod(
      ...
      if (classIds.length === 0) {
        ...
        const empty = {
          ...
          atRiskStudents: [],
          genderBreakdown: {
            male: { count: 0, passRatePercent: 0 },
            female: { count: 0, passRatePercent: 0 },
          },
          studentFlow: {
      ...
      // ── Overview ──
      const classMap = new Map(
        classes.map((c) => [c.id, { name: c.name, grade: c.grade }]),
      );
      ...
      const femaleTeachers = teacherRecords.filter(
        (t) => t.gender === "FEMALE",
      ).length;
      ...
      studentSubjectMeans.forEach((subjectMeansForStudent, studentId) => {
        const entries = Array.from(subjectMeansForStudent.entries()).map(
          ([subjectId, mean]) => ({
            mean,
      ...
      const topStudentsInClass = scopeClassId
        ? (
            ...
            .filter(
              (s) => studentInfoMap.get(s.studentId)?.classId === scopeClassId,
            )
            .sort((a, b) => b.average - a.average)
            .slice(0, 5)
            .map((s, index) => ({ ...s, rank: index + 1 }))
      ...
      const atRiskStudents = Array.from(averageMap.entries())
        ...
        .map(([studentId, average]) => {
          const info = studentInfoMap.get(studentId);
          const classInfo = info?.classId
            ? classMap.get(info.classId)
            : undefined;
          return {
            studentId,
            name: info?.name || "",
      ...
      const genderBreakdown = {
        male: {
          ...
          passRatePercent: genderTotals.get("MALE")?.count
            ? Math.round(
                ((genderTotals.get("MALE")?.passing || 0) /
                  (genderTotals.get("MALE")?.count || 1)) *
                  100,
              )
            : 0,
        },
        female: {
          ...
          passRatePercent: genderTotals.get("FEMALE")?.count
            ? Math.round(
                ((genderTotals.get("FEMALE")?.passing || 0) /
                  (genderTotals.get("FEMALE")?.count || 1)) *
                  100,
              )
            : 0,
        },
      ...
      studentSubjectMeans.forEach((subjectMeansForStudent, studentId) => {
        ...
        subjectMeansForStudent.forEach((mean, subjectId) => {
          ...
          const bucket = subjectAgg.get(key) || {
            ...
            failing: 0,
            gradeCounts: new Map(
              GRADE_LETTERS.map((g) => [g, { total: 0, male: 0, female: 0 }]),
            ),
          };
          const percentage =
            meta.maxScore > 0 ? (mean / meta.maxScore) * 100 : 0;
      ...
      const responseBody = {
        ...
        bottomPerformingClasses: schoolWide
          ? averageScoreByClass.slice(-5).reverse()
          : [],
