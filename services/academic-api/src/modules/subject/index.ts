...
app.use('/subjects', authenticateToken);
  ...
  try {
    ...
    const subjects = await prisma.subject.findMany({
      ...
      include: {
        ...
        academicYearOfferings: typeof academicYearId === 'string' && academicYearId ? {
...
app.post('/subjects', authenticateToken, requireSubjectAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const schoolId = getSchoolId(req);
    if (academicYearId) {
      const ownedYear = await prisma.academicYear.count({
    ...
    const subject = await prisma.$transaction(async (tx) => {
      ...
      if (academicYearId) {
        await tx.academicYearSubject.create({
          data: { academicYearId, subjectId: createdSubject.id, isActive: true },
        });
      }
