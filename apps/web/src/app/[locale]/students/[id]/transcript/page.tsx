...
export default function StudentTranscriptPage() {
  ...
  const [activeTab, setActiveTab] = useState<"cumulative" | "monthly">(
    "cumulative",
  );
  const [selectedMonths, setSelectedMonths] = useState<
    { month: string; monthNumber: number; year: number; classId: string }[]
  >([]);
  ...
  const availableMonths = useMemo(() => {
    ...
    const addRecord = (
      ...
      const monthLabel =
        getKhmerMonthLabel(monthNumber) || month || `Month ${monthNumber}`;
      ...
      academicYear.subjects.forEach((subject) => {
        subject.grades.forEach((grade) =>
          addRecord(
            grade.monthNumber,
            grade.year,
            grade.classId || academicYear.classId,
            grade.month,
          ),
        );
      });
    ...
    const academicOrder = new Map<number, number>(
      KHMER_MONTHS.map((month, index): [number, number] => [
        month.number,
        index,
      ]),
    );
    return [...records.values()].sort((a, b) => {
      ...
      return (
        (academicOrder.get(a.monthNumber) ?? 99) -
        (academicOrder.get(b.monthNumber) ?? 99)
      );
    });
  }, [transcript]);
    ...
    async (signal?: AbortSignal) => {
      try {
        ...
        const response = await fetch(
          `${STUDENT_SERVICE_URL}/students/${studentId}/transcript`,
          {
            headers: {
        ...
        if (
          response.ok &&
          data?.success &&
          Array.isArray(data?.data?.academicYears)
        ) {
          ...
          throw new Error(
            data?.error || `Failed to load transcript (${response.status})`,
          );
        }
      } catch (err: any) {
    ...
    const fetchSchoolProfile = async () => {
      ...
      try {
        ...
        if (res.success && res.data) {
          ...
          const savedSettingsRaw = localStorage.getItem(
            `${SETTINGS_STORAGE}:${school.id}`,
          );
          let savedSettings = {};
          if (savedSettingsRaw) {
            try {
          ...
          setSettings((prev) => ({
            ...
            province: res.data.province
              ? `ខេត្ត${res.data.province}`
              : (savedSettings as any).province || prev.province,
            teacherName: (savedSettings as any).teacherName || prev.teacherName,
            principalName:
              (savedSettings as any).principalName || prev.principalName,
    ...
    const promises = monthsToFetch.map(async (m) => {
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            locale === "km"
              ? "មិនមានទិន្នន័យសិស្សក្នុងរបាយការណ៍ខែនេះទេ"
              : "This student is missing from the monthly report",
          );
        }
        ...
        console.error(
          `Failed to load monthly report for ${m.month} ${m.year}:`,
          err,
        );
        setFetchErrors((prev) => ({
          ...prev,
          [key]: err.message || "Failed to load report",
        }));
        setLoadingMonths((prev) => ({ ...prev, [key]: false }));
      }
    });
  ...
  const handleRetryMonth = async (m: {
    month: string;
    monthNumber: number;
    year: number;
    classId: string;
    ...
    try {
      ...
      console.error(
        `Failed to retry monthly report for ${m.month} ${m.year}:`,
        err,
      );
      setFetchErrors((prev) => ({
        ...prev,
  ...
  const selectedAcademicYear =
    transcript?.academicYears.find((year) => year.yearId === selectedYearId) ??
    transcript?.academicYears[0];
  const selectedYearDocument = selectedYearId
    ? transcript?.documentMetaByYear?.[selectedYearId]
    : null;
  ...
  const handleIssueOfficial = async () => {
    ...
    try {
      ...
      const token = TokenManager.getAccessToken();
      const response = await fetch(
        `${STUDENT_SERVICE_URL}/students/${studentId}/transcript/issue`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!response.ok || !data?.success) {
        throw new Error(
          data?.error || `Failed to issue transcript (${response.status})`,
        );
      }
      setTranscript((prev) => {
        if (!prev) return prev;
      ...
      setIssueError(
        err.message ||
          (locale === "km"
            ? "មិនអាចចេញឯកសារផ្លូវការបានទេ"
            : "Could not issue official transcript"),
      );
    } finally {
  ...
  const handleRevokeOfficial = async () => {
    ...
    try {
      ...
      const token = TokenManager.getAccessToken();
      const response = await fetch(
        `${STUDENT_SERVICE_URL}/students/${studentId}/transcript/revoke`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          body: JSON.stringify({
            academicYearId: selectedAcademicYear.yearId,
            reason,
          }),
        },
      );
      ...
      if (!response.ok || !data?.success) {
        throw new Error(
          data?.error || `Failed to revoke transcript (${response.status})`,
        );
      }
      setTranscript((prev) => {
        if (!prev) return prev;
      ...
      setIssueError(
        err.message ||
          (locale === "km"
            ? "មិនអាចដកហូតឯកសារផ្លូវការបានទេ"
            : "Could not revoke official transcript"),
      );
    } finally {
        ...
        >
          <div className="mx-auto max-w-6xl space-y-6 animate-pulse">
            ...
            <div className="flex items-center gap-3 text-gray-600 dark:text-gray-300">
              ...
              />
              <span>
                <AutoI18nText i18nKey="auto.web.students_id_transcript_page.k_62cb9bc3" />
              </span>
            </div>
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 flex items-center gap-4">
            ...
            <div>
              <h3 className="font-semibold text-red-800">
                <AutoI18nText i18nKey="auto.web.students_id_transcript_page.k_f18cbf09" />
              </h3>
              ...
              <div className="mt-3 flex flex-wrap gap-4">
                ...
                >
                  ...
                  <AutoI18nText i18nKey="auto.web.students_id_transcript_page.k_0751d601" />
                </button>
              </div>
            </div>
          </div>
  ...
  const renderCumulativeCard = (isPrint: boolean) => {
    ...
    const selectedYear =
      academicYears.find((y) => y.yearId === selectedYearId) ||
      academicYears[0];
    if (!selectedYear) return null;
    const officialDocument =
      transcript.documentMetaByYear?.[selectedYear.yearId];
    ...
    const schoolName =
      schoolProfile?.nameKh || schoolProfile?.name || school?.name || "";
    ...
    const sumWhenComplete = (values: Array<number | null>) =>
      values.length > 0 && values.every((value) => value !== null)
        ...
        : null;
    const totalMax = sumWhenComplete(
      subjectsData.map((subject) => subject.maxScore),
    );
    const totalS1 = sumWhenComplete(
      subjectsData.map((subject) => subject.s1Score),
    );
    const totalS2 = sumWhenComplete(
      subjectsData.map((subject) => subject.s2Score),
    );
    const totalAnnual = sumWhenComplete(
      subjectsData.map((subject) => subject.annualScore),
    );
    ...
    const overallAvgPct =
      annualOverallAvg === null ? null : annualOverallAvg * 2;
    const overallGrade =
      overallAvgPct === null ? null : getLetterGrade(overallAvgPct);
    const overallRemark = overallGrade ? getKhmerRemark(overallGrade) : null;
    const overallResult =
      overallAvgPct === null ? null : overallAvgPct >= 50 ? "ជាប់" : "ធ្លាក់";
    ...
    return (
      <div className="khmer-transcript-print w-full bg-white text-black rounded-xl">
        ...
        `}</style>
        <div
          className="transcript-page cumulative-transcript-page bg-white text-black border border-gray-200 shadow-sm"
          style={{
            width: isPrint ? "210mm" : "100%",
            minHeight: isPrint ? "297mm" : "auto",
            padding: isPrint ? "5mm" : "15px",
        ...
        >
          ...
          <div
            className="cumulative-transcript-inner border-[3px] border-double border-black p-4 h-full flex flex-col justify-between"
            style={{ minHeight: isPrint ? "287mm" : "auto" }}
          >
            <div
              className={`mb-2 border px-3 py-1.5 text-center text-[9px] font-bold ${
                isOfficialDocument
            ...
            <div>
              ...
              <div className="transcript-header-container flex justify-between items-start mb-3 relative">
                <div
                  className="transcript-header-left text-left flex-1"
                  style={{ paddingTop: logoUrl ? "0px" : "40px" }}
                >
                  {logoUrl && (
                    <div className="mb-1">
                      <img
                  ...
                  <p className="transcript-moul-branding text-xs font-semibold text-black">
                    ...
                    {cleanProvince ? `ខេត្ត${cleanProvince}` : "..."}
                  </p>
                  <p className="transcript-moul-branding text-xs font-semibold text-black">
                ...
                <div
                  className="transcript-header-right text-center w-[200px]"
                  style={{
                    position: "absolute",
                    left: "50%",
                ...
                >
                  <p className="transcript-kingdom-text text-sm font-semibold text-black">
                    ព្រះរាជាណាចក្រកម្ពុជា
                  </p>
                  <p className="transcript-kingdom-text text-sm font-semibold mt-0.5 text-black">
                    ជាតិ សាសនា ព្រះមហាក្សត្រ
                  </p>
                  <p className="transcript-symbol-3 text-xl mt-0.5 text-black">
                    3
                  </p>
                </div>
                ...
                {/* Photo Box */}
                <div
                  className="transcript-photo-placeholder w-[70px] h-[90px] border border-gray-400 flex flex-col items-center justify-center bg-gray-50 rounded select-none"
              ...
              <div className="text-center mb-3">
                <h2
                  className="text-lg font-bold text-red-600 uppercase"
                ...
                {isOfficialDocument && (
                  <p className="mt-1 text-[9px] font-semibold text-black">
                    Document No: {officialDocument?.documentNumber} ·
                    Verification: {officialDocument?.verificationCode}
                  </p>
                )}
              </div>
              ...
              <div className="border border-black p-3 rounded-lg mb-3 bg-transparent text-black">
                <div className="grid grid-cols-2 gap-x-6 gap-y-1.5 text-[11px]">
                  ...
                  <div>
                    ...
                    <span className="font-bold">
                      {toKhmerNumerals(student.studentId || "N/A")}
                    </span>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <span className="font-bold">
                      {toKhmerNumerals(
                        selectedYear.className || summary.currentClass || "N/A",
                      )}
                    </span>
                  </div>
                </div>
              ...
              <div className="mb-3">
                <h3
                  className="text-xs font-bold text-black border-b border-black pb-0.5 mb-2"
                  style={{ fontFamily: "var(--font-moul), serif" }}
                >
                  ក. លទ្ធផលនៃការសិក្សា
                </h3>
                ...
                <table className="cumulative-table w-full border-collapse border border-black text-[9px] text-black">
                  <thead>
                    <tr className="bg-gray-150 text-center font-bold">
                      <th
                        className="border border-black p-0.5 w-[4%]"
                        rowSpan={2}
                      >
                        ល-រ
                      </th>
                      <th
                        className="border border-black p-0.5 text-left pl-1"
                        rowSpan={2}
                      >
                        មុខវិជ្ជា
                      </th>
                      <th
                        className="border border-black p-0.5 w-[8%]"
                        rowSpan={2}
                      >
                        ពិន្ទុអតិបរមា
                      </th>
                      ...
                      <th
                        className="border border-black p-0.5 w-[36%]"
                        colSpan={5}
                      >
                        លទ្ធផលប្រចាំឆ្នាំសិក្សា
                      </th>
                      <th
                  ...
                  <tbody>
                    {subjectsData.map((subject, idx) => (
                      <tr
                        ...
                        className="hover:bg-gray-55 text-center"
                      >
                        <td className="border border-black p-0.5 font-bold">
                          {toKhmerNumerals(idx + 1)}
                        </td>
                        <td className="border border-black p-0.5 text-left pl-1 font-medium">
                          {subject.subjectNameKh || subject.subjectName}
                        </td>
                        <td className="border border-black p-0.5">
                          {subject.maxScore === null
                            ? "-"
                            : toKhmerNumerals(subject.maxScore)}
                        </td>
                        <td className="border border-black p-0.5 font-semibold">
                          {displayNumber(subject.s1Score, 1)}
                        </td>
                        <td className="border border-black p-0.5 text-gray-700">
                          -
                        </td>
                        <td className="border border-black p-0.5 font-semibold">
                          {displayNumber(subject.s2Score, 1)}
                        </td>
                        <td className="border border-black p-0.5 text-gray-700">
                          -
                        </td>
                        <td className="border border-black p-0.5 font-bold">
                          {displayNumber(subject.annualScore)}
                        </td>
                        <td className="border border-black p-0.5 text-gray-700">
                          -
                        </td>
                    ...
                    <tr className="bg-gray-100 font-bold text-center">
                      ...
                      <td className="border border-black p-0.5">
                        {displayNumber(totalMax, 0)}
                      </td>
                      <td className="border border-black p-0.5">
                        {displayNumber(totalS1, 1)}
                      </td>
                      <td className="border border-black p-0.5 bg-gray-50">
                        -
                      </td>
                      <td className="border border-black p-0.5">
                        {displayNumber(totalS2, 1)}
                      </td>
                      <td className="border border-black p-0.5 bg-gray-50">
                        -
                      </td>
                      <td className="border border-black p-0.5">
                        {displayNumber(totalAnnual)}
                      </td>
                      <td
                        className="border border-black p-0.5 bg-gray-50"
                        colSpan={4}
                      >
                        -
                      </td>
                    ...
                    <tr className="font-semibold text-center">
                      ...
                      <td className="border border-black p-0.5">
                        {displayNumber(s1ExamAvg)}
                      </td>
                      <td className="border border-black p-0.5 text-gray-700">
                        -
                      </td>
                      <td className="border border-black p-0.5">
                        {displayNumber(s2ExamAvg)}
                      </td>
                      <td className="border border-black p-0.5 text-gray-700">
                        -
                      </td>
                      <td className="border border-black p-0.5 font-bold">
                        {displayNumber(annualExamAvg)}
                      </td>
                      <td
                        className="border border-black p-0.5 text-gray-750"
                        colSpan={5}
                      >
                        -
                      </td>
                    ...
                    <tr className="font-semibold text-center">
                      <td
                        ...
                        colSpan={2}
                      >
                        មធ្យមភាគពិន្ទុប្រចាំឆមាស
                      </td>
                      <td className="border border-black p-0.5">៥០</td>
                      <td className="border border-black p-0.5">
                        {displayNumber(s1MonthlyAvg)}
                      </td>
                      <td className="border border-black p-0.5 text-gray-700">
                        -
                      </td>
                      <td className="border border-black p-0.5">
                        {displayNumber(s2MonthlyAvg)}
                      </td>
                      <td className="border border-black p-0.5 text-gray-700">
                        -
                      </td>
                      <td className="border border-black p-0.5 font-bold">
                        {displayNumber(annualMonthlyAvg)}
                      </td>
                      <td
                        className="border border-black p-0.5 text-gray-750"
                        colSpan={5}
                    ...
                    <tr className="font-bold text-center bg-gray-100">
                      <td
                        ...
                        colSpan={2}
                      >
                        មធ្យមភាគពិន្ទុរួមប្រចាំឆ្នាំ
                      </td>
                      <td className="border border-black p-0.5">៥០</td>
                      <td className="border border-black p-0.5">
                        {displayNumber(s1OverallAvg)}
                      </td>
                      <td className="border border-black p-0.5 text-gray-700">
                        -
                      </td>
                      <td className="border border-black p-0.5">
                        {displayNumber(s2OverallAvg)}
                      </td>
                      <td className="border border-black p-0.5 text-gray-700">
                        -
                      </td>
                      <td className="border border-black p-0.5">
                        {displayNumber(annualOverallAvg)}
                      </td>
                      <td className="border border-black p-0.5 text-gray-700">
                        -
                      </td>
                      ...
                      <td
                        ...
                        style={{
                          color:
                            overallResult === "ធ្លាក់" ? "#dc2626" : "inherit",
                        }}
                      >
                        {overallResult || "-"}
                      </td>
                ...
                <table className="cumulative-attendance-table w-full text-center text-[9px] border-collapse border border-black text-black">
                  <thead>
                    <tr className="bg-gray-150 font-bold">
                      <th className="border border-black p-0.5 w-[25%]">
                        អវត្តមាន
                      </th>
                      <th className="border border-black p-0.5 w-[25%]">
                        ប្រចាំឆមាសទី១
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td className="border border-black p-0.5">-</td>
                      <td className="border border-black p-0.5 font-bold">
                        {displayAttendance(annualExcused)}
                      </td>
                    </tr>
                    <tr>
                      <td className="border border-black p-0.5 font-medium">
                      ...
                      <td className="border border-black p-0.5">-</td>
                      <td className="border border-black p-0.5 font-bold">
                        {displayAttendance(annualUnexcused)}
                      </td>
                    </tr>
                  </tbody>
                </table>
            ...
            <div className="transcript-signatures-container flex justify-between mt-4 text-[10px] text-black">
              <div className="signature-column text-center w-[200px]">
                ...
                <p
                  className="signature-title font-bold mt-0.5"
                  style={{ fontFamily: "var(--font-moul), serif" }}
                >
                  នាយកសាលា
                </p>
                ...
                <p
                  className="signature-name font-bold"
                  style={{ fontFamily: "var(--font-moul), serif" }}
                >
                  {settings.principalName || "________________"}
                </p>
              </div>
              ...
              <div className="signature-column text-center w-[200px]">
                ...
                <p
                  ...
                  style={{ fontFamily: "var(--font-moul), serif" }}
                >
                  {settings.teacherName || "________________"}
                </p>
              </div>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-800/50">
      ...
      <main className="lg:ml-64 p-4 lg:p-8 print:ml-0 print:p-0">
        ...
        <nav className="flex items-center text-sm text-gray-600 mb-6 print:hidden">
          ...
          <ChevronRight className="w-4 h-4 mx-2" />
          <button
            onClick={() => router.push(`/${locale}/students/${studentId}`)}
            className="hover:text-orange-600"
          >
          ...
          <span className="text-gray-900 dark:text-white font-medium">
            {locale === "km" ? "ព្រឹត្តិបត្រពិន្ទុ" : "Academic transcript"}
          </span>
        </nav>
          ...
          <div className="flex gap-2">
            {activeTab === "cumulative" && (
              <>
                <button
                  ...
                  title={
                    !canIssueOfficial && !isSelectedYearOfficial
                      ? locale === "km"
                        ? "ត្រូវការសិទ្ធិ admin និងពិន្ទុដែលបានរក្សាទុក"
                        : "Admin access and saved grades are required"
                      : undefined
                  }
                ...
                {isSelectedYearOfficial &&
                  ["ADMIN", "SUPER_ADMIN", "SCHOOL_ADMIN"].includes(
                    userRole,
                  ) && (
                    <button
            ...
            <button
              ...
              title={
                !canPrint
                  ? locale === "km"
                    ? "មិនទាន់មានទិន្នន័យគ្រប់គ្រាន់សម្រាប់បោះពុម្ព"
                    : "Complete data is required before printing"
                  : undefined
              }
            ...
            <button
              ...
              title={
                !canPrint
                  ? locale === "km"
                    ? "មិនទាន់មានទិន្នន័យគ្រប់គ្រាន់សម្រាប់រក្សាទុក PDF"
                    : "Complete data is required before saving a PDF"
                  : undefined
              }
        ...
        {issueError && (
          <div
            className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 dark:border-red-900/60 dark:bg-red-950/30 dark:text-red-200 print:hidden"
            role="alert"
          >
            {issueError}
          </div>
        ...
        <div
          className={`mb-6 rounded-xl border px-4 py-3 text-sm print:hidden ${
            isSelectedYearOfficial
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-semibold">
                {isSelectedYearOfficial
                  ? locale === "km"
                    ? "ឯកសារផ្លូវការបានចេញរួច"
                    : "Official transcript issued"
                  : locale === "km"
                    ? "ឯកសារព្រាងសម្រាប់ពិនិត្យ"
                    : "Review draft"}
              </p>
              <p
                className={`mt-0.5 ${isSelectedYearOfficial ? "text-emerald-800 dark:text-emerald-300" : "text-amber-800 dark:text-amber-300"}`}
              >
                {isSelectedYearOfficial
                  ? locale === "km"
              ...
              {isSelectedYearOfficial &&
                selectedYearDocument?.verificationCode && (
                  <button
                    onClick={() =>
                      router.push(
                        `/${locale}/transcripts/verify/${selectedYearDocument.verificationCode}`,
                      )
                    }
        ...
        <div
          className="flex border-b border-gray-200 dark:border-gray-700 mb-6 print:hidden"
          role="tablist"
          aria-label={
            locale === "km" ? "ប្រភេទព្រឹត្តិបត្រពិន្ទុ" : "Transcript type"
          }
        >
          <button
          ...
          <div className="flex flex-col lg:flex-row gap-6 print:hidden">
                    ...
                    >
                      {academicYears.map((y) => (
                        <option
                          key={y.yearId}
                          value={y.yearId}
                          className="dark:bg-gray-950 text-black dark:text-white"
                        >
                          {y.yearName}
                        </option>
                      ))}
            ...
            <div className="flex-1 bg-white dark:bg-gray-900 rounded-xl shadow-md p-6 border border-gray-200 dark:border-gray-800 min-h-[500px] flex flex-col items-center justify-start overflow-auto">
              {canPrintCumulative ? (
                ...
                <div
                  className="my-auto max-w-md py-20 text-center"
                  role="status"
                >
                  ...
                  <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                    {locale === "km"
                      ? "មិនទាន់មានពិន្ទុសម្រាប់ឆ្នាំសិក្សានេះ"
                      : "No grades for this academic year yet"}
                  </h2>
            ...
            <div className="w-full lg:w-80 space-y-6">
              ...
              <div className="bg-white dark:bg-gray-900 rounded-xl shadow-md p-5 border border-gray-200 dark:border-gray-800">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">
                  {tTranscript("select_month_to_print") ||
                    "Select month(s) to print:"}
                </h3>
                  ...
                  >
                    {Object.values(loadingMonths).some(Boolean) ? (
                      <>
                        ...
                        <span>
                          {locale === "km" ? "កំពុងបង្កើត..." : "Generating..."}
                        </span>
                      </>
                    ) : (
                      <>
                  ...
                  {generatedMonths.map((m) => {
                    ...
                    if (report) {
                      return (
                        <div
                          key={`${m.classId}-${m.monthNumber}-${m.year}`}
                          className="scale-[0.8] origin-top md:scale-100 shadow-lg bg-white rounded-lg w-full max-w-[210mm]"
                        >
                          <TranscriptPrint
                            report={report.data}
                            settings={settings}
                    ...
                    if (isLoading) {
                      return (
                        <div
                          key={`${m.classId}-${m.monthNumber}-${m.year}`}
                          className="w-full max-w-[210mm] aspect-[1/1.414] bg-white dark:bg-gray-900 rounded-lg shadow-md border border-gray-200 dark:border-gray-800 p-8 flex flex-col justify-between animate-pulse"
                        >
                          {/* Skeleton Header */}
                    ...
                    if (errorMsg) {
                      return (
                        <div
                          key={`${m.classId}-${m.monthNumber}-${m.year}`}
                          className="w-full max-w-[210mm] aspect-[1/0.5] bg-white dark:bg-gray-900 rounded-lg shadow-md border border-red-200 dark:border-red-900/50 p-8 flex flex-col items-center justify-center text-center gap-4"
                        >
                          ...
                          <div>
                            <h5 className="font-bold text-gray-800 dark:text-gray-200 mb-1">
                              {locale === "km"
                                ? `ការបង្កើតបរាជ័យសម្រាប់ខែ${m.month}`
                                : `Failed to load ${m.month}`}
                            </h5>
