      ...
      <div className="lg:ml-64 min-h-screen">
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-8">
          <div className="flex items-center gap-4">
            <div className={`w-16 h-16 bg-white dark:bg-gray-900/20 rounded-2xl ${shimmer}`} />
            <div>
              <div className={`h-8 bg-white dark:bg-gray-900/30 rounded-lg w-48 mb-2 ${shimmer}`} />
              <div className={`h-4 bg-white dark:bg-gray-900/20 rounded w-64 ${shimmer}`} />
            </div>
          </div>
        </div>
        ...
        <div className="p-8">
          {/* Filter Skeleton */}
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border p-6 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[1, 2, 3].map((i) => (
                <div key={i}>
                  <div className={`h-4 bg-gray-200 rounded w-24 mb-2 ${shimmer}`} />
                  <div className={`h-10 bg-gray-100 dark:bg-gray-800 rounded-lg ${shimmer}`} />
                </div>
              ))}
            </div>
          </div>
            ...
            {[
              ...
              <div key={i} className={`bg-gradient-to-br ${gradient} rounded-2xl p-5`}>
                <div className="flex items-center justify-between">
                  <div>
                    <div className={`h-3 bg-white dark:bg-gray-900/30 rounded w-20 mb-2 ${shimmer}`} />
                    <div className={`h-8 bg-white dark:bg-gray-900/40 rounded w-16 ${shimmer}`} />
                  </div>
                  <div className={`w-12 h-12 bg-white dark:bg-gray-900/20 rounded-xl ${shimmer}`} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6 mb-6">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border p-6">
                <div className={`h-6 bg-gray-200 rounded w-40 mb-4 ${shimmer}`} />
                <div className={`h-[300px] bg-gray-100 dark:bg-gray-800 rounded-lg ${shimmer}`} />
              </div>
            ))}
          </div>
              ...
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="flex items-center gap-4 py-2">
                  ...
                  <div className="flex-1">
                    ...
                    <div className={`h-3 bg-gray-100 dark:bg-gray-800 rounded w-24 ${shimmer}`} />
                  </div>
