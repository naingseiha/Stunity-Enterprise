...
export default function CompactHeroCard({
  ...
  return (
    <div
      className={`relative h-full overflow-hidden rounded-[1.75rem] border border-white/70 p-6 shadow-[0_30px_70px_rgba(15,23,42,0.1)] ${backgroundClassName}`}
    >
      <div className={`absolute inset-y-0 right-0 w-72 ${glowClassName}`} />
      <div className="relative flex h-full flex-col">
        {breadcrumbs ? <div>{breadcrumbs}</div> : null}

        <div className="mt-6 flex flex-1 flex-col gap-5 md:flex-row md:items-start md:justify-between">
          <div className="max-w-3xl">
            <div className="flex items-center gap-4">
              <div className={`rounded-[1.2rem] px-4 py-4 shadow-[0_16px_30px_rgba(15,23,42,0.16)] ${iconShellClassName}`}>
                <Icon className="h-7 w-7" />
              </div>
              <div>
                <p className={`text-[11px] font-black uppercase tracking-[0.32em] ${eyebrowClassName}`}>{eyebrow}</p>
                <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950 md:text-[2.2rem]">{title}</h1>
              </div>
            </div>
