...
const TYPE_CONFIG = {
  pdf: { icon: FileText, color: 'text-rose-500', bg: 'bg-rose-50 dark:bg-rose-900/20' },
  video: { icon: Video, color: 'text-violet-500', bg: 'bg-violet-50 dark:bg-violet-900/20' },
  link: { icon: Link2, color: 'text-teal-500', bg: 'bg-teal-50 dark:bg-teal-900/20' },
  doc: { icon: BookOpen, color: 'text-[#F9A825]', bg: 'bg-amber-50 dark:bg-amber-900/20' },
};
    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
      ...
      <div className="px-4 py-3 flex items-center justify-between border-b border-gray-50 dark:border-gray-800">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-semibold text-gray-900 dark:text-gray-100 text-sm">{tFeed('widgets.quickResources.title')}</h3>
            <p className="text-xs text-gray-400 dark:text-gray-500">{tFeed('widgets.quickResources.recentFiles')}</p>
          </div>
        </div>
      </div>
        ...
        {QUICK_RESOURCES.map((resource) => {
          ...
          return (
            <button
              ...
              className="w-full p-3 flex items-center gap-3 hover:bg-amber-50/30 dark:hover:bg-amber-900/10 transition-all duration-300 group text-left"
            >
              {/* Icon */}
              <div className={`
              ...
              <div className="flex-1 text-left min-w-0">
                <div className="flex items-center gap-2">
                  <h4 className="font-medium text-gray-900 dark:text-gray-100 text-sm truncate group-hover:text-[#F9A825] transition-colors">
                    {resource.title}
                  </h4>
                  {resource.isNew && (
                    <span className="px-1.5 py-0.5 text-[10px] font-bold bg-amber-100 dark:bg-amber-900/40 text-[#F9A825] rounded-full">
